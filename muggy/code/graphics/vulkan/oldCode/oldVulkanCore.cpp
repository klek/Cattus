    ...
    namespace // Anonymous namespace, ie only for use in this cpp-file
    {
        ...
        public:
                                     ...
                                     uint32_t swapchainImageCount )
            {
                ...
                // TODO(klek): Add copy queue in the future
                ...
                //****************************************************
                // TODO(klek): THIS IS FOR DEBUGGING ONLY
                // NOTE(klek): This data should come from the application
                // vertex::vertices.push_back( { { 0.0f, -0.5f }, { 1.0, 0.0, 0.0 } } );
                // vertex::vertices.push_back( { { 0.0f, 0.5f }, { 0.0, 1.0, 0.0 } } );
                // vertex::vertices.push_back( { { -0.0f, 0.5f }, { 0.0, 0.0, 1.0 } } );

                // vertex::createVertexBuffer( vertex::vertices,
                //                             vertexBuffer,
                //                             vertexBufferMemory,
                //                             m_CmdPool,
                //                             m_GraphicsQueue );
            ...
            bool endFrame( vulkan_surface* surface )
            {
                ...
                // vkCmdDraw( cmdBuffer.cmdBuffer, static_cast<uint32_t>( vertex::vertices.size() ), 1, 0, 0 );
                ...
                // TODO(klek): Consider adding memory barriers for
                //             when using different queues for
            ...
            void release( void )
            {
                ...
                //****************************************************
                // TODO(klek): THIS IS FOR DEBUGGING ONLY
                // vertex::destroyVertexBuffer( vertexBuffer, vertexBufferMemory );
                //****************************************************
        ...
        private:
            void createCmdBuffers( VkDevice device, [[maybe_unused]] uint32_t queueFamilyIndex )
            {
                ...
                for ( uint32_t i{ 0 }; i < m_SwapchainImageCount; i++ )
                {
                    ...
                    {
                        // NOTE(klek): Freeing command buffers
                        freeCmdBuffer( device, m_CmdPool, m_CmdBuffers[ i ] );
                    }

                    // NOTE(klek): Allocate a new command buffer from
                    //             the pool
                    if ( !allocateCmdBuffer( device, m_CmdPool, true, m_CmdBuffers[ i ] ) )
                    {
                        MSG("Failed to allocate cmdBuffer in pool...");
                        // TODO(klek): Should we handle error here?
                    }
                }
            }
            ...
            {
                VkBuffer        cmdBuffer{ VK_NULL_HANDLE };
        ...
        bool isDeviceSuitable( VkPhysicalDevice device, VkSurfaceKHR surface )
        {
            ...
            if ( extSupported )
            {
                ...
                bool result = getSwapchainDetails( device, surface, swapchainDetails );
                assert( result );
        ...
        // NOTE(klek): This function is an example
